...
int main()
{
	...
	for(int i=1;i<=n;i++)
	{
		if(i+1<=n) ins(night(i) , night(i + 1) , INF , 0);//每天晚上可以将脏餐巾留到第二天晚上
		if(i+t1<=n) ins(night(i) , day(i + t1) , INF , m1);//每天晚上可以送去快洗部,在地i+t1天早上收到餐巾
		if(i+t2<=n) ins(night(i) , day(i + t2) , INF , m2);//每天晚上可以送去慢洗部,在地i+t2天早上收到餐巾
		ins(st , day(i) , INF , m);//每天早上可以购买餐巾
	}
	printf("%lld",flow());
